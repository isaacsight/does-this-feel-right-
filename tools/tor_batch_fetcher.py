...
def generate_index(catalog):
    ...
    tmp_idx = INDEX_PATH.with_suffix(".md.tmp")
...
def main():
    ...
    print(f"TOR Batch Fetcher started for {len(pending_items)} pending items.")
    ...
    for item in pending_items:
        ...
        for attempt in range(1, 4):
            try:
                ...
                print(f"[{completed_count}] Saved {v_id}: {title[:40]}")
                break
            except Exception as e:
                err = str(e)
    ...
    print("TOR Batch Fetcher completed!")
