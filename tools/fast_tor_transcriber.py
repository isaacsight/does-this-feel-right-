...
def process_video_task(item):
    ...
    if file_path.exists() and file_path.stat().st_size > 100:
        ...
        return v_id, "completed", str(file_path.relative_to(BASE_DIR)), w_count
...
def main():
    ...
    # First sync disk files to catalog state
    disk_completed = 0
    ...
    print(f"Synced {disk_completed} disk transcripts. Starting High-Speed Parallel TOR Engine for {len(pending_items)} pending items...", flush=True)
    
    workers = 5
    processed_count = 0
    
    with ThreadPoolExecutor(max_workers=workers) as executor:
        future_to_item = {executor.submit(process_video_task, item): item for item in pending_items}
        
        for future in as_completed(future_to_item):
            item = future_to_item[future]
            v_id = item["id"]
            try:
                vid, status, rel_file, word_count = future.result()
                with catalog_lock:
                ...
                processed_count += 1
                if status == "completed":
                    print(f"[{processed_count}/{len(pending_items)}] Completed {vid}: {item['title'][:35]}", flush=True)
                else:
                    print(f"[{processed_count}/{len(pending_items)}] Status ({status}) {vid}: {item['title'][:35]}", flush=True)
            except Exception as e:
                print(f"Error on {v_id}: {e}", flush=True)
    ...
    save_catalog(catalog)
    generate_index(catalog)
    print("Parallel TOR Engine completed!", flush=True)
