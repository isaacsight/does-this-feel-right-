...
## Resolution and style — corrected 2026-07-30

**The first analysis here was wrong and is recorded so nobody repeats it.**

Gemini's in-page `<img>` is a downscaled preview at ~1024px. Measuring that
preview produced the conclusion that every frame would need a 1.875x upscale,
and the canvas was dropped to 1280x720 and halftone abandoned to compensate.

The **downloaded** file is **2752x1536**. There is no upscaling problem at all:
1920x1080 is a 1.43x *downscale* from source, which is the good direction.

Always measure the downloaded asset, never the page preview.

**What the film actually does:**

1. **Canvas is 1920x1080**, downscaled from ~2752px sources.
2. **Flat colour fills, no halftone** — retained, but as an *aesthetic* choice
   for this film rather than a technical workaround. It gives this film its own
   surface against the last one. Anyone restoring halftone later is making a
   style decision, not fixing a bug.

**Two instructions Gemini reliably ignores**, both found on the reference sheet
and both requiring a per-batch audit rather than trust:

- "no text" — it labelled the sheet anyway
- "full bleed, no margin" — the sheet trims to `2602x1440+70+84`
