...
| 413 | ledger | classic | — | seal: HANDED OVER · PRINT YOUR OWN · VII·26 | amethyst | press (new) | Sixth interaction shape — Press: the first artifact control; the reader operates the real production grammar (six stocks, eleven POPEYE-safe seeds, three layouts, their own headline/seal/number) and a cover assembles live; the law travels with the instruments — off-grammar covers impossible by construction, not validation; colophon states the composition and grades nothing (411's ethic at higher stakes); session-only, print to keep — the artifact leaves with the reader, numbered 001 | `press` spread type; thirteenth editorial tool; the composing rung — the ladder complete: watch→choose→walk→cut→learn→write→COMPOSE |
| 414 | ivory | asymmetric-left | — | seal: RECEIPTS OVER RATINGS · VII·26 | brick | dispatch | External-convergence dispatch: a recorded talk (Peter Steinberger, on the record) describes three independently-built tools — transcript-attachment, auto-review-to-origin-session, disposable VNC sandboxes — that land on kernel.chat's own doctrine (raw trace over self-reported score); verbatim account of a faked 2→5 PR rating as rule-6's villain in the wild; bridges to 401's "verify by event, not handshake" | first dispatch whose subject is an external party's independent convergence with house doctrine, not the house's own operational record |
| 415 | ink | monument-hero | full-stop | — | oxblood | close (new) | Seventh interaction shape — Close: a feed with no natural end until the reader gives it one; "Show me one more" and "I'll stop here" as siblings at identical weight from item one, enforced by one shared class and a regression test; session clock from mount; the soft cap is part of the argument | `close` spread type; `full-stop` ornament (a heavy period bleeding off the cover — the most literal image of what the issue hands the reader) |
| 416 | ivory | asymmetric-left | — | seal: SPECULATIVE STUDY · VII·26 | tomato | instrument | Speculative model study run as an honest instrument: GPT 5.6 SOL's register dial (PRECISE/EXPANSIVE/SOULFUL) turned into a real gemma3:12b run, measured off the ollama API; the warm answer was not the expensive one | optional `dialLabel` field on `instrument` (a dial can be honestly labelled something other than Effort) |
| 417 | ivory | classic | — | seal: PROOF · MARKED BY HAND · VII·26 | pool | proof (new) | Eighth interaction shape — Proof: the correction pass; a machine-completed draft (real gemma3:12b, 6 lines, verbatim, raw JSON filed) adjudicated line by line — KEEP MACHINE / TAKE THE HAND / STRIKE — composing a resolved screen + provenance ledger; rule 6 doubled (reader marks session-only AND machine lines never hand-edited, disclosed in `machineNote`); completes galley→proof→press | `proof` spread type; first spread whose base text is machine-authored; the print-shop production line closed |
| 418 | butter | classic | — | seal: READER AS SUBJECT · VII·26 | cobalt | day (new) | Ninth interaction shape — Day: an authored metropolitan day (06:10–00:40), nine delegation moments marked LET IT RIDE / STEP IN; both authored consequences always legible (rule 4 without branching); midnight ledger meters only the reader's marks, changes of mind, and the session clock; behavioral-science file cited for real (Bainbridge 83 · Parasuraman/Riley 97 · Skitka 99 · Lee/See 04); ambient city vignettes at weather amplitude with the `--day-depth` tint | `day` spread type; first shape whose axis is time lived; first reader-as-subject issue (N = 1, unrecorded) |
| 419 | kraft | asymmetric-left | — | seal: WORKING MODEL · VII·26 | ivy | plate (new) | Tenth interaction shape — Plate: a working model of FLORA's intelligent canvas (field review, facts read from flora.ai 2026-07-15); four blocks wired on a framed plate, PULL THE PROOF walks the signal down the wires and each frame draws a seeded botanical engraving — in-house simulation, disclosed three times (dossier, plateNote, caption); first issue to exercise interaction-language.md's amendment clause (rule 3 working-model exception: script-driven signal inside the frame, timer-robust, reduced-motion collapsed; rule 4 seeded reproducibility: the seed printed on the artwork is the state's address, resting state seeded from the issue number) | `plate` spread type; first mechanism control; first `ivy` accent issue; registration-tick plate frame |
| 420 | ink | classic | — | seal: CORE SAMPLE No.1 · VII·26 | pool | bore (new) | Eleventh interaction shape — Bore: the depth control, born from its reservation by editorial promotion of the artifact that demonstrated it; a probe lowered through six strata of 416's real prompt (surface → type case → constellation → loom → dice → return) with carried context (the picked-up sort re-inks the plates below), a temperature radiogroup, and the answer winched up; complies with the depth doctrine (artifact-language §III): named axis + gauge, carried context, a floor that pays, emphasis-never-existence, one honest probe; strata disclosed as drawn (no model run), real works cited (Vaswani 17 · Mikolov 13 · Sennrich 16 · Holtzman 20) | `bore` spread type; first depth control; first artifact-first issue under the ratified artifact law; the page darkens with the reader's depth (`--live-depth`) |
| 421 | ivory | classic | — | seal: FREQUENCY SUM · VIII·26 | celadon | fourier (new) | Twelfth interaction shape — Fourier: the waveform summation control, and the first audible one; additive synthesis operated live — base-shape radiogroup (sine/sawtooth/square/triangle), partials slider (1–24), inharmonicity B modelling string stiffness (f_n = n·f₀·√(1+B·n²)), and direct phase-dragging on a phasor constellation; the synthesized wave plotted in real time and heard only on the reader's press, through a Web Audio bank hard-capped at 0.05 gain; all math computed in-house, ledger counts only the reader's adjustments; real works cited (Roads 01 · Chowning 73 · Zavalishin 12); inherits the working-model exception (timer-robust script inside the rig frame, reduced-motion draws once and rests) | `fourier` spread type; first audible control; `celadon` declared over the fourier pool default (second celadon issue after 394); circle radius carries the coefficient's magnitude, its sign folds into the arm (a negative Fourier weight is a phase inversion, not a negative length) |
| 422 | ivory | classic | — | seal: ATTENTION AUDIT No.1 · IX·26 | tomato | audit (new) | Thirteenth interaction shape — Audit: the SESSION control; the reader's own attention is the axis. Simon's 1971 warning staged as an operable treasury: five strata (Wealth → Scarcity → Allocation → Zone → Floor), a beam that raises emphasis where the reader's hand rests, a hundred-unit budget sealed and CARRIED (the zone machine's magnetism is geared to the reader's own feed grant), the 415 stop-as-sibling law in the zone, and a reconciliation floor: declared vs revealed, measured live, kept by no one; real works cited (Simon 71 · Goldhaber 97 · Schüll 12 · Wu 16 · Williams 18) | `audit` spread type; FIRST MERGER ISSUE — apparatus whole, no reduction (artifact-language §I amended, rule 3 apparatus register); first spread whose gauge meters the reader's own session; beam collapses to full ink under reduced-motion and touch-only hands |
| 423 | ledger | asymmetric-left | — | seal: ROUTING PLATE No.2 · X·26 | pool | plate | Kimi K3 architecture review built around the durable operating facts rather than benchmark rank: 2.8T parameters, 896 experts, 16 active, 1M context, KDA + Attention Residuals + Stable LatentMoE, preserved thinking history, and supernode-scale deployment; the artifact leads with a named depth axis and carried operating brief, while the site lawfully reduces it to the existing framed block-and-wire plate; every expert field is deterministic and explicitly illustrative | second `plate` instance establishes the working-model pattern; optional `proofStyle: 'routing'` varies the proof grammar without adding a fourteenth interaction shape; first issue to use the shortened `coverDeck` as a deliberate poster line |
| 424 | ink | monument-hero | — | seal: LOCAL RECEIPT No.1 · XI·26 | amethyst | plate | GALLEY’s localhost production route staged as brief → model → estimate → decision → receipt; the quote precedes the charge and Palmier receives a local file | third `plate` instance; deterministic studio contact-sheet grammar, approval and refusal held at equal weight |
| 426 | ink | ledger-rule | — | seal: DISPATCH DESK · NIGHT SHIFT · I·27; postmark: THE DISPATCH DESK · I·27 | cobalt | forecast | An operable dispatch board: the reader files jobs, a seeded cobalt sky decides their fates, and the floor reproduces the real July 19 job registry — 37 departures printed as found, seven running and six queued at press time; split-flap settle, congestion gauge, session ledger, printed seed 426-0719 | first `forecast` used literally (a weather report for compute); third interaction shape after the 422 instrument and 424 configurator; first ink cover in the modern index; four cyanotype plates commissioned, one printed (back cover) |
| 425 | kraft | asymmetric-left | — | seal: CUT No.1 · HUMAN FINISH · XII·26; postmark: THE EDIT ROOM · XII·26 | oxblood | sequence | Five reader-controlled stations carry ONE OF ONE from call → measure → make → fit → street; each stage pairs a verified production receipt with an original issue-only mixed-media plate, never a film frame; tabs gain previous/next traversal, 420/560ms paper-settle choreography, and reduced-motion collapse; artifact adds a scroll-progress thread and CSS view-timeline plate movement | second `sequence` instance; optional stage-image module; first issue to treat a completed film as source while refusing reproduction of the film itself |
| 429 | ivory | classic | warty-spots | seal: SPECIMEN 00 · NO RUN YET · III·27 | ivy | plate | Fourth `plate` instance — a materials review of the Oxford Nanopore MinION written in the specimen register: nothing bought, no run performed, declared on the cover seal and enforced line by line (store prices read 2026-08-18; run figures third-party — Menon, 2026-05 — marked ◇ wherever printed); 369's warty-spots ornament re-read as pores; artifact edition descends bench → extraction → library → pore → basecall → folio with the sample choice and basecall model carried down; the pre-press fact pass caught a false "all open source" stack claim and corrected it in the source brief in the same change | `squiggle` proof grammar (fourth plate proof: a stepped ionic-current trace with a seeded base-call window raised into the accent); first ivy-accent plate; first hardware-purchase-brief issue; price tag denominated in the run ($1,100 · THE INK) |
...
- **Japanese subtitles + parallel text**: Noto Serif JP → `var(--font-jp)`

No generic UI sans-serif. The Japanese companion is a language-specific
voice, not a third Latin display register.
