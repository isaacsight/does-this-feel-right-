...
async function tryLocalFirst(message: string): Promise<string | null> {
  ...
  const looksArithmetic =
    /\b(?:what\s+is|what['']s|compute|calculate|evaluate)\b/i.test(lower)
    || /^[-\d.\s+*×/÷%()?]+$/.test(lower)
...
Always quote file paths that contain spaces. Never reference internal system names.`
  ...
  try {
    ...
    if (preResult.systemPromptAddition) coordinatorContext = preResult.systemPromptAddition
  ...
  try {
    ...
    if (coordinatorContext) systemContext += `\n\n${coordinatorContext}`
  ...
  const pipeline = options.pipeline ?? createDefaultPipeline({
    ...
    executeTool: async (name, args) => {
      ...
      try {
        ...
        const eval_ = coord.evaluateToolCall(name, args as Record<string, unknown>)
        if (eval_.warn) process.stderr.write(`\n  \x1b[2m[coordinator] ${eval_.warn}\x1b[0m\n`)
      } catch { /* non-critical */ }
