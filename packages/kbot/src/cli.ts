...
async function main(): Promise<void> {
  ...
  skillsCmd
    ...
    .action(async () => {
      ...
      if (skills.length === 0) { printInfo('No skills found. Run `kbot skills import --from hermes` to import 76 Hermes skills.'); return }
  ...
  skillsCmd
    ...
    .action(async (opts: { from?: string }) => {
      ...
      if (!existsSync(src)) {
        ...
        if (from === 'hermes') printInfo('Install Hermes: `ollama launch hermes --yes --model hermes3:8b`')
        return
      }
      const result = await importExternalSkills(src)
      printSuccess(`Imported ${result.imported} skill${result.imported === 1 ? '' : 's'} from ${src}`)
      if (result.skipped > 0) printInfo(`  ${result.skipped} skipped (existing user-authored files preserved)`)
  ...
  memoryCmd
    ...
    .action(async (opts: { maxAgeDays?: string; obsoleteVersion?: string; dryRun?: boolean }) => {
      ...
      if (result.backup) printSuccess(`Backup written: ${result.backup}`)
  ...
  program
    ...
    .action(async (briefWords: string[], opts: { out?: string; kind?: string; pdf?: boolean; open?: boolean }) => {
      ...
      try {
        const out = await runDesign({
          brief,
          out: opts.out,
          kind: (opts.kind as any) || 'one-pager',
          pdf: opts.pdf,
          open: opts.open,
        })
        ...
        if (!opts.open) printInfo(`Preview: open ${out}`)
      } catch (err) {
