...
function makeMachineProfile(overrides: Partial<MachineProfile> = {}): MachineProfile {
  return {
    model: 'MacBook Pro',
    ...
    memory: { total: '128 GB', totalBytes: 137438953472, free: '64 GB', freeBytes: 68719476736, used: '64 GB', usedBytes: 68719476736, pressure: 'low' as const },
...
describe('gatherContext', () => {
  ...
  it('detects a git repo and extracts branch', () => {
    mockExecSync.mockImplementation((cmd: unknown) => {
      const c = String(cmd)
      if (c.includes('rev-parse --show-toplevel')) return '/home/user/project\n'
      if (c.includes('branch --show-current')) return 'feat/context\n'
      if (c.includes('git diff --stat')) return ' 3 files changed\n'
      return ''
    })
    mockReaddirSync.mockReturnValue([])
  ...
  it('detects TypeScript + React from package.json', () => {
    mockExecSync.mockImplementation((cmd: unknown) => {
      const c = String(cmd)
      if (c.includes('rev-parse --show-toplevel')) return '/project\n'
      if (c.includes('branch --show-current')) return 'main\n'
      return ''
    })
  ...
  it('detects Svelte, Express, and Fastify frameworks', () => {
    const frameworks: Array<{ dep: string; expected: string }> = [
  ...
  it('detects package manager from lock files', () => {
    const lockFiles: Array<{ file: string; expected: string }> = [
    ...
    for (const { file, expected } of lockFiles) {
      vi.clearAllMocks()
      mockExecSync.mockImplementation((cmd: unknown) => {
        if (String(cmd).includes('rev-parse --show-toplevel')) return '/project\n'
        return ''
      })
      mockExistsSync.mockImplementation((p: unknown) => {
        const path = String(p)
  ...
  it('detects Python from pyproject.toml', () => {
    mockExecSync.mockReturnValue('')
    mockExistsSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith('pyproject.toml')) return true
      return false
    })
    mockReaddirSync.mockReturnValue([])
  ...
  it('detects Rust from Cargo.toml', () => {
    mockExecSync.mockReturnValue('')
    mockExistsSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith('Cargo.toml')) return true
      return false
    })
    mockReaddirSync.mockReturnValue([])
  ...
  it('detects Go from go.mod', () => {
    mockExecSync.mockReturnValue('')
    mockExistsSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith('go.mod')) return true
      return false
    })
    mockReaddirSync.mockReturnValue([])
