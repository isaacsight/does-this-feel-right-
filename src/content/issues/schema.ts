/* ──────────────────────────────────────────────────────────────
   kernel.chat — Issue schema

   Every type the editorial system is built from: the cover fields,
   the 20-member `IssueSpread` discriminated union, and the
   `IssueRecord` shape each issue file satisfies. Split out of
   index.ts (ISSUE-era god-file) so the archive/registry — the issue
   imports and the ALL_ISSUES array everyone appends to — evolves and
   merges independently of the schema.

   To add a new editorial tool: add its interface + `type` literal
   here, extend the IssueSpread union, then register the component in
   src/components/IssueFeature.tsx (its exhaustiveness check enforces
   the wiring at compile time). See src/content/issues/PUBLISHING.md §V.
   ────────────────────────────────────────────────────────────── */
