...
import { V5 as FRAMES, V5_FIX, V5_FILL } from './frames-v5.mjs'
...
const VALIDATE = ['v22', 'v03']
const BATCH = 20

let pick, tag
if (mode === '--fill') { pick = V5_FILL; tag = 'fill' }
else if (mode === '--fix') { pick = V5_FIX; tag = 'fix' }
...
writeFileSync(join(OUT, `jobs-v5-${tag}.json`), JSON.stringify(jobs, null, 2))
console.log(`\nwritten jobs-v5-${tag}.json · spend $${(jobs.length * 0.08).toFixed(2)}`)
console.log(`poll: node videos/polyvagal/poll.mjs jobs-v5-v2-${tag}.json`)
