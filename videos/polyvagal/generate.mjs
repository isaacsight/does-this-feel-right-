...
import { RESHOOTS, OPOSSUM, PACING, FILL } from './frames.mjs'
const mode = process.argv[2] || '--estimate'
const VALIDATE = ['f01', 'f35']
const BATCH = 20

let pick, tag
if (mode === '--fill') { pick = FILL; tag = 'fill' }
else if (mode === '--pacing') { pick = PACING; tag = 'pacing' }
else if (mode === '--opossum') { pick = OPOSSUM; tag = 'opossum' }
else if (mode === '--reshoot') { pick = RESHOOTS; tag = 'reshoot' }
...
writeFileSync(join(OUT, `jobs-${tag}.json`), JSON.stringify(jobs, null, 2))
console.log(`\nwritten jobs-${tag}.json · spend $${(jobs.length * 0.08).toFixed(2)}`)
console.log(`poll: node videos/polyvagal/poll.mjs jobs-${tag}.json`)
