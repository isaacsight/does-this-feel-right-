...
# The board numbers its lines with register-profile.py's splitter (a bare
# [.!?]+ split, which also breaks "$2.13" into two fragments — the board
# stages those fragments deliberately). The map must count the same way or
# every frame after the first decimal lands one line early.
sents = [s.strip() for s in
         re.split(r'[.!?]+', (FILM / 'script.txt').read_text()) if s.strip()]
...
for n, sent in enumerate(sents, 1):
    toks = norm(sent)
    hit = None
    for ln in range(min(8, len(toks)), 1, -1):
        probe = toks[:ln]
...
# Decimal fragments: the splitter broke "33.9 days" into two lines, but the
# recording speaks one token. Anchor each fragment line to the literal token so
# the data card appears exactly when its number is said.
FRAGMENT_ANCHORS = {19: '33.9', 21: '55.5.', 30: '48.6', 32: '55.5.'}
used = set()
for n, tok in sorted(FRAGMENT_ANCHORS.items()):
    for i, w in enumerate(words):
        if w['word'] == tok and i not in used:
            sent_idx[n] = i; used.add(i); break
...
# The first picture covers the lead-in silence: without this, shot 1 lasts
# only from its first word to the next line's first word (0.17s on this film).
shots[0]['start'] = 0.0
shots[0]['dur'] = round(max(0.7, shots[0]['end']), 3)
