# WORLD — risograph, two inks, WITH cast
...
## THIS FILM HAS A CAST — the decision reversed, and why

`who-you-measured` went cast-free because the world test failed the protagonist:
he came back an unreadable blob in the wide, and in close-up his two fine hairs
rendered as insect antennae with the whole head reading as a bug.

**That was a fact about the DESIGN, not about the medium.** Real risograph carries
characters constantly — they are simply drawn for it. A six-image cast test on
2026-08-04 ($0.23) put the house protagonist back in this world with his shapes
made bold, and he survives decisively: readable small in the wide, expressive in
extreme close-up, and seven of him stay distinct in one frame.

Two faults that test found, both now fixed in the clause:

**HAIR — thick strokes read as RABBIT EARS.** Thin was antennae; thick was ears.
Weight was the wrong axis. The fix is SHAPE: the two strokes **kink and lean at
different angles and are different lengths**, like a cowlick. Never two straight
vertical strokes, never symmetrical.

**GREEN leaked in as a third ink** on a leaf. In a two-ink world that is a world
violation, and the gate CANNOT catch it — the hue census is disabled here by
necessity (see below). So it is forbidden by name in every prompt, along with
yellow, orange and brown, and every leaf and vine prints in pink or blue.

The character maps onto two inks almost natively, which is why the crossover works:
tomato-red hood to the PINK, black trousers to the BLUE, cream face and shoes left
as bare paper.

## The printed panel — kept on purpose

Every frame in the first cast batch came back with a band of bare paper around the
inked area, like a panel printed on a page. Nothing asked for it. Isaac liked it,
so it stays — but it is now SPECIFIED rather than left to chance. An accidental
border that varies frame to frame is the ledger world's margin rule all over again:
a handsome effect the model cannot place consistently reads as sloppiness.

**The model cannot place it consistently.** Two smoke frames came back with panels
of visibly different proportions — one wide, one nearly square. So the clause asks
for a FULL BLEED frame and `tools/video/panel.py` insets it onto the paper margin
afterwards, identically, every time. This is the stair-tread ruling applied again:
anything the model cannot place reliably gets drawn deterministically instead.

`ground_min_paper` is set to 0.02 because this film's environments are dense and
inked edge to edge inside the panel. The ground check now validates the paper's
COLOUR wherever paper is visible, instead of demanding that paper dominate — two
different claims, and only the first is a rule of this world.
