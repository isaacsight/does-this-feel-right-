...
ROW = re.compile(r'^\|\s*\*\*(b\d\d)\*\*\s*\|\s*(.*?)\s*\|\s*([\d.]+)\s*\|\s*([^|]+)\|', re.M)
rows = ROW.findall(board)

frames = []
for fid, line, dur, motion in rows:
    line = re.sub(r'^\*+"?|"?\*+$', '', line.strip()).strip().strip('"')
    frames.append({'id': fid, 'line': line,
                   'board_dur': float(dur), 'motion': motion.strip()})
frames.sort(key=lambda f: int(f['id'][1:]))

want = [f'b{i:02d}' for i in range(1, 89)]
...
for i, f in enumerate(frames):
    ...
    shots.append({'id': f['id'], 'start': round(start, 3), 'end': round(end, 3),
                  ...
                  'board_dur': f['board_dur'], 'motion': f['motion'],
                  'line': f['line'][:70]})
...
drift = [abs(s['dur'] - s['board_dur']) for s in shots]
print(f'{len(shots)} shots  total {total:.1f}s')
print(f'  hold  min {min(d):.2f}  mean {sum(d)/len(d):.2f}  max {max(d):.2f}')
print(f'  vs board  mean drift {sum(drift)/len(drift):.2f}s  worst {max(drift):.2f}s')
if unmatched:
